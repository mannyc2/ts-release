...
const run = (command: string, args: ReadonlyArray<string>, cwd: string) => spawnSync(command, [...args], { cwd, encoding: "utf8", stdio: "pipe" })
...
try {
  ...
  if (cli.status !== 0 || !/^0\.2\.0\n?$/u.test(cli.stdout.trim())) failures.push("The Node CLI bundle did not report candidate version 0.2.0.")
  const action = run("node", [join(root, "apps/ts-release-action/dist/index.js")], root)
  if (action.status === 0 || !`${action.stdout}\n${action.stderr}`.includes("Action command must be one of")) failures.push("The Action bundle did not execute its parser under Node.")
