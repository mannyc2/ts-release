...
if (!workflow.includes("command: release")) failures.push("The automatic release workflow does not invoke release once.")
for (const command of ["prepare", "inspect", "publish"]) if (workflow.includes(`command: ${command}`)) failures.push(`The automatic release workflow incorrectly invokes ${command}.`)
if (workflow.includes("actions/upload-artifact@v4") || workflow.includes("actions/download-artifact@v4")) failures.push("The automatic release workflow duplicates the Action's durable store.")
