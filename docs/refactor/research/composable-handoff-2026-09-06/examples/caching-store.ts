...
import type { AppendResult, JournalEvent, JournalStore, LabError, Snapshot } from "../machine/src/index.js"
...
export class CachingJournalStore implements JournalStore {
  ...
  invalidate(journalId?: string): void {
    ...
    if (journalId === undefined) this.cache.clear(); else this.cache.delete(journalId)
  }
  read = (journalId: string): Effect.Effect<Snapshot, LabError> => Effect.suspend(() => {
    this.counters.reads++
    const cached = this.cache.get(journalId)
    if (cached) return Effect.succeed({ revision: cached.revision, events: cached.events.slice() })
    this.counters.underlyingReads++
    return this.underlying.read(journalId).pipe(Effect.tap((snapshot) => Effect.sync(() => {
      this.cache.set(journalId, { revision: snapshot.revision, events: snapshot.events.slice() })
    })))
  })
  append = (journalId: string, expectedRevision: number, event: JournalEvent): Effect.Effect<AppendResult, LabError> => Effect.suspend(() => {
    this.counters.appends++
    return this.underlying.append(journalId, expectedRevision, event).pipe(
      Effect.tap((result) => Effect.sync(() => {
        const cached = this.cache.get(journalId)
        if (result._tag === "Appended" && cached !== undefined && cached.revision === expectedRevision && result.revision === expectedRevision + 1) {
          this.cache.set(journalId, { revision: result.revision, events: [...cached.events, event] })
