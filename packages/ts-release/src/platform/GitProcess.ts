...
const command = (executable: string, directory: string, options: GitProcessOptions): RawCommand =>
  Effect.fn("git.command")(function* (args, input, environment = {}) {
    ...
    return yield* Effect.callback<GitResult, ReleaseError>((resume) => {
      ...
      return Object.freeze({
        ...
        repository: Effect.fn("git.openRepository")(function* (format: "sha1" | "sha256") {
          ...
          const directory = yield* attempt(() => mkdtempSync(join(root, "repository-"))),
            execute = command(options.gitExecutable, directory, options)
