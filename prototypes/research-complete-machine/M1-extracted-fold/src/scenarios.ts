import { ExtractedFoldApplication } from "./machine.js"
import {
  bool,
  integer,
  sortedFacts,
...
const scenarioActions: Record<CaseId, ReadonlyArray<ActionId>> = {
...
export const runCase = (invocation: CaseInvocation): CaseObservation => {
  ...
  const application = new ExtractedFoldApplication()
  const actions = scenarioActions[invocation.caseId]
  const trace = actions.map((actionId, index) => {
    application.apply(actionId, invocation.fixture)
    const snapshot = application.snapshot
    const facts: Array<readonly [string, ReturnType<typeof bool>]> = [
    ...
    if (index === actions.length - 1) facts.push(["trace.terminal-outcome", text(application.terminalOutcome)])
...
export const selfCheck = (): void => {
  if (Object.keys(scenarioActions).length !== 16) throw new Error("not all frozen cases are implemented")
  if (scenarioActions["C14-finalized-file-tree-adoption"][0] !== "action.adopt-finalized-artifacts") throw new Error("artifact boundary case is not wired")
  if (scenarioActions["C16-journal-bound-symmetry"].length !== 5) throw new Error("bounded-store case is incomplete")
}
