export const CASE_IDS = [
  "C01-initial-success", "C02-rejection-before-commit", "C03-response-loss-satisfied-observation",
  "C04-response-loss-inconclusive-stop", "C05-core-git-cas-protected-replay",
  "C06-explicit-risk-acceptance", "C07-concurrent-runners-single-cas-winner",
  "C08-request-endpoint-mismatch", "C09-supersession-late-evidence",
  "C10-ambiguous-append-readback", "C11-malformed-provider-graph",
  "C12-external-provider-two-instances", "C13-apple-commit-before-id-loss",
  "C14-finalized-file-tree-adoption", "C15-host-dependency-shadowing",
...
export type ActionId =
  | "action.initialize-operation" | "action.prepare-operation" | "action.validate-provider-graph"
  | "action.append-dispatch-authority" | "action.dispatch-operation"
  | "action.record-provider-receipt" | "action.observe-operation"
  | "action.derive-terminal-report" | "action.record-precommit-rejection"
  | "action.resume-fresh-runner" | "action.request-risk-acceptance"
  | "action.append-risk-acceptance" | "action.contend-append-cas"
  | "action.refold-after-cas-loss" | "action.verify-request-correspondence"
  | "action.append-supersession" | "action.append-late-evidence"
  | "action.reconcile-ambiguous-append" | "action.load-external-provider"
  | "action.submit-apple-operation" | "action.adopt-finalized-artifacts"
  | "action.attempt-host-shadow" | "action.write-journal-at-limit"
  | "action.read-journal-at-limit" | "action.reject-journal-write-over-limit"
...
export const FORBIDDEN_TRANSITIONS = [
  "Ready->derive-terminal-report", "Planned->derive-terminal-report",
  "Authorized->derive-terminal-report", "RiskAccepted->derive-terminal-report"
] as const

export interface ProviderPlugin {
  readonly id: string
  readonly instances: ReadonlyArray<{ readonly id: string; readonly endpointClass: string }>
  readonly prepare: (instanceId: string, requestId: string) => { readonly providerId: string; readonly instanceId: string; readonly requestId: string }
}
...
export const canonicalStringify = (value: unknown): string => {
  const visit = (item: unknown): unknown => Array.isArray(item) ? item.map(visit) : item !== null && typeof item === "object"
    ? Object.fromEntries(Object.keys(item as Record<string, unknown>).sort(codePointCompare).map((key) => [key, visit((item as Record<string, unknown>)[key])]))
    : item
...
export const sortedFacts = (facts: ReadonlyArray<readonly [string, EvidenceValue]>): ReadonlyArray<EvidenceEntry> => [...facts].sort(([a], [b]) => codePointCompare(a, b)).map(([name, value], index) => ({ name, sequence: index + 1, value }))
...
class Machine {
  ...
  apply(action: ActionId, fixture: CaseFixture): void {
    if (action === "action.derive-terminal-report") {
      if (this.state === "Satisfied") this.terminalOutcome = "Succeeded"
      else if (this.state === "Rejected") this.terminalOutcome = "Rejected"
      else if (this.state === "Superseded" || this.state === "SafeStopped") this.terminalOutcome = "SafeStop"
      else if (this.state === "InFlight" || this.state === "Unproven") this.terminalOutcome = "Inconclusive"
      else throw new Error(`terminal report requested from nonterminal state ${this.state}`)
      return
    }
    switch (action) {
      case "action.initialize-operation":
        if (hasFault(fixture, action, "fault.prior-dispatch-response-loss")) this.record("InFlight")
        if (hasFault(fixture, action, "fault.prior-inconclusive-operation")) this.record("Unproven")
        return
      ...
      case "action.dispatch-operation": case "action.submit-apple-operation":
        if (this.state !== "Authorized") throw new Error("dispatch attempted without current authority")
        this.dispatchCount += 1; this.record("InFlight"); return
      ...
      case "action.observe-operation":
        if (hasFault(fixture, action, "fault.observation-absence") || hasAnyFault(fixture, "fault.apple-submission-id-loss")) { this.record("Unproven"); return }
        this.observationCount += 1
        if ((hasAnyFault(fixture, "fault.prior-dispatch-response-loss") || hasAnyFault(fixture, "fault.prior-inconclusive-operation")) && this.observationCount === 1) this.record("Unproven", true)
        else this.record("Satisfied", true)
        return
      case "action.request-risk-acceptance": if (this.state !== "Unproven") throw new Error("risk acceptance without unproven evidence"); return
      ...
      case "action.refold-after-cas-loss": if (this.state !== "Authorized") throw new Error("CAS loser did not refold"); return
      case "action.verify-request-correspondence": if (!hasFault(fixture, action, "fault.request-endpoint-mismatch")) throw new Error("missing mismatch fault"); this.record("Rejected"); return
      case "action.append-supersession": this.record("Superseded", true); return
      case "action.append-late-evidence": this.record("Superseded", true); return
      case "action.reconcile-ambiguous-append": if (!hasFault(fixture, "action.append-dispatch-authority", "fault.append-outcome-unknown")) throw new Error("missing ambiguous append"); this.record("SafeStopped", true); return
      ...
      case "action.reject-journal-write-over-limit": case "action.reject-journal-read-over-limit": if (this.journalLimit <= 0) throw new Error("bounded store missing"); return
    }
  }
}

export const runCase = (invocation: CaseInvocation) => {
  if (invocation.fixture.caseId !== invocation.caseId) throw new Error("case and fixture identifiers differ")
  const machine = new Machine()
  const actions = CASE_ACTIONS[invocation.caseId]
  const trace = actions.map((actionId, index) => {
    machine.apply(actionId, invocation.fixture)
    const facts: Array<readonly [string, EvidenceValue]> = [
      ["trace.fresh-runner", bool(machine.freshRunner)], ["trace.journal-revision", integer(machine.revision)],
      ["trace.observation-count", integer(machine.observationCount)], ["trace.provider-dispatch-count", integer(machine.dispatchCount)]
    ]
    for (const fault of invocation.fixture.faultSchedule) if (fault.actionId === actionId) facts.push([`trace.${fault.faultId}`, bool(true)])
    if (index === actions.length - 1) facts.push(["trace.terminal-outcome", text(machine.terminalOutcome!)])
    return { actionId, facts: sortedFacts(facts), sequence: index + 1 }
  })
  return {
    schemaVersion: "architecture-case-observation-v2", runContextSha256: invocation.runContextSha256,
    candidateId: invocation.candidateId, candidateTreeSha256: invocation.candidateTreeSha256,
    definitionSha256: invocation.definitionSha256, caseId: invocation.caseId,
    fixtureSha256: invocation.fixtureSha256, trace,
    facts: sortedFacts([["summary.action-count", integer(actions.length)], ["summary.fault-count", integer(invocation.fixture.faultSchedule.length)], ["summary.final-journal-revision", integer(machine.revision)], ["summary.observation-count", integer(machine.observationCount)], ["summary.provider-dispatch-count", integer(machine.dispatchCount)]]),
    terminalOutcome: machine.terminalOutcome!
  }
}
