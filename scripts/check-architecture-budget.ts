...
const legacy = new Map<string, string>(waves.migrationExecutionMapping.currentSourceRows.map((row: { path: string; sha256: string }) => [row.path, row.sha256]))
for (const input of forecast.legacyGeneratedInputs) legacy.set(input.path, input.sha256)
const counts = { product: 0, unchangedLegacy: 0, replacement: 0, metadata: 0, tests: 0, generatedDelivery: 0, tooling: 0 }
...
for (const path of paths) {
  ...
  const product = source && (path.startsWith("src/") || /^(?:apps|packages)\/[^/]+\/src\//.test(path)) || /^apps\/[^/]+\/action\.yml$/.test(path)
  const lane = product ? "product" : /(?:^|\/)test(?:s)?\//.test(path) ? "tests"
    : /(?:^|\/)dist\//.test(path) ? "generatedDelivery"
    : source && /^(?:tools|scripts|prototypes)\//.test(path) ? "tooling"
    : /(?:package|tsconfig[^/]*)\.json$/.test(path) ? "metadata" : undefined
  if (!lane) continue
  const bytes = readFileSync(resolve(root, path))
  const lines = bytes.length === 0 ? 0 : bytes.filter((byte) => byte === 10).length + (bytes.at(-1) === 10 ? 0 : 1)
...
if (process.argv.includes("--write")) await Bun.write(resolve(root, "docs/refactor/execution/source-measurement.json"), JSON.stringify({ wave, counts, files }, null, 2) + "\n")
if (counts.replacement > limit || wave === "W10" && counts.product > 11485) {
  throw new Error("Source tripwire exceeded: simplify implementation or record a concrete tradeoff; the ceiling is unchanged")
}
