...
const build = {
  id: optional(Schema.String), targets: Schema.Array(target),
...
export const CandidateBuild = Schema.Union([
  CandidateBunBuild, CandidateCommandBuild, CandidatePrebuiltBuild
])
...
export class CandidateWheel extends Schema.Class<CandidateWheel>("CandidateWheel")({
  id: OutputId, path: SafeRelativePath, wheelTag: Schema.String,
  binaries: Schema.Array(CandidateWheelBinary)
}) {}
...
export class CandidateArchive extends Schema.Class<CandidateArchive>("CandidateArchive")({
  ...
  formats: optional(Schema.Array(format)),
  formatOverrides: optional(Schema.Struct({
    linux: optional(Schema.Array(format)), darwin: optional(Schema.Array(format)),
    windows: optional(Schema.Array(format))
  })),
  files: optional(Schema.Array(nonempty)),
...
const hook = {
  id: nonempty, run: Schema.NonEmptyArray(nonempty), cwd: optional(SafeRelativePath),
  env: optional(Schema.Array(nonempty))
}
...
export class CandidateHooks extends Schema.Class<CandidateHooks>("CandidateHooks")({
  before: optional(Schema.Array(CandidateBeforeHook)),
  after: optional(Schema.Array(CandidateRiskHook))
}) {}

const trusted = {
  provider: optional(Schema.Literal("github-actions")), workflow: optional(nonempty)
}
export class CandidateNpmPublish extends Schema.Class<CandidateNpmPublish>("CandidateNpmPublish")({
  ...
  trustedPublishing: optional(Schema.Struct({
    ...trusted, verifyPackageExists: optional(Schema.Boolean)
  })),
...
export class CandidatePyPiPublish extends Schema.Class<CandidatePyPiPublish>("CandidatePyPiPublish")({
  ...
  trustedPublishing: optional(Schema.Struct({
    ...trusted, publisherConfigured: optional(Schema.Literal(true))
  })),
...
{}
const preset = {
  repository: Schema.String, ids: optional(Schema.NonEmptyArray(OutputId)),
...
export class CandidateConfig extends Schema.Class<CandidateConfig>("CandidateConfig")({
  ...
  retry: optional(Schema.Struct({ attempts: Schema.Number, delayMillis: Schema.Number })),
  evidence: optional(Schema.Union([
    SafeRelativePath, Schema.Struct({ directory: SafeRelativePath })
  ]))
}) {}
