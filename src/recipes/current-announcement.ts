import { AnnouncementPublish, PublishCredential } from "../model/operation.js"
...
import { announcementHttpProfiles } from "./announcement-profiles.js"
...
export const lowerCurrentAnnouncements = (config: CandidateConfig, rows: CurrentRows): void => {
  ...
  for (const action of config.publish.announce ?? []) {
    if (action.profileId === "announce.index/v1" || action.profileId === "announce.smtp/v1") continue
