...
export const providerProfile = <const I extends Input>(i: I) => Object.freeze({
  ...
  checkpoints: i.checkpoints ?? ["upload"], contract: Object.freeze({
    kind: i.kind ?? "repository-publish", variant: i.variant ?? "RepositoryPublish", protocolVersion: i.protocol,
    targetCoordinates: i.target, allowedOptions: i.options ?? ["contentType"],
    ...
    request: { method: i.method, baseUrl: i.base, pathTemplate: i.path,
      ...
      bodySchema: i.body ?? "verified-content-stream/v1", reconciliationKeyLocation: "x-ts-release-key" },
    response: { successSchema: i.success ?? "repository-item/v1", errorSchema: "provider-error/v1",
    ...
    classification, reconciliation: { supported: i.reconcile !== "NONE", method: i.reconcile,
      pathTemplate: i.reconcilePath ?? i.path, equality: i.equality },
