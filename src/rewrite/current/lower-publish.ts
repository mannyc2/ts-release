...
const lowerNpm = (config: CandidateConfig, rows: CurrentRows) => {
  ...
  return PackageRegistryRelease.make({
    ...
    registryUrl: section.registry ?? "https://registry.npmjs.org",
...
const lowerPyPi = (config: CandidateConfig, rows: CurrentRows) => {
  ...
  return PackageRegistryRelease.make({
    ...
    registryUrl: section.repositoryUrl ?? "https://upload.pypi.org/legacy/",
    packagePath: path("."), artifactPaths: artifacts.map((item) => item.path),
    clientExecutable: section.pythonExecutable ?? "python",
    trustedPublishing: oidc, ...trusted(section.trustedPublishing),
