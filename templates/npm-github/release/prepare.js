...
export const prepareRelease = Effect.fn("release.prepare")(function* (raw) {
    ...
    const retain = Effect.fn("release.retain")(function* (logicalName, bytes, mediaType) {
    ...
    for (const entry of input.packages) {
        ...
        if (metadata.private || metadata.version !== input.version)
            return yield* failure("package-cohort", "Every selected package must be public at the selected version");
        packages.push({ file, metadata });
    }
    if (new Set(packages.map((entry) => entry.metadata.name)).size !== packages.length ||
        (input.corePackage !== undefined &&
            !packages.some((entry) => entry.metadata.name === input.corePackage)))
        return yield* failure("package-cohort", "Package names must be unique and include the selected core");
    ...
    for (const { file, metadata } of packages) {
        let retainedProvenance = new Npm.NoProvenance({});
        if (provenance && attester) {
            ...
            retainedProvenance = new Npm.GitHubActionsProvenance({
                source: provenance.source,
                bundle: yield* retain(`${file.logicalName}.sigstore.json`, attestation.bytes, attestation.mediaType),
                mediaType: attestation.mediaType,
            });
        }
        intents.push(new Npm.PublishIntent({
    ...
    if (core)
        npm.push(yield* Npm.publish(core, npm.map((operation) => operation.operationId)));
    const tag = yield* GitHub.lightweightTag(new GitHub.LightweightTag({
        repository,
        tag: `v${input.version}`,
        commit: source.commit,
        principal: GITHUB_PRINCIPAL,
    }), npm.map((operation) => operation.operationId));
    const draft = yield* GitHub.draft(new GitHub.DraftIntent({
        repository,
        tag: `v${input.version}`,
    ...
    const plan = yield* createPlan(bundleSha256, [...npm, tag, draft, ...assets, publication], releaseJournalId(source));
    const noRead = () => Effect.fail(failure("prepare-network", "Plan admission cannot read a publication destination"));
    yield* loadPlan(plan, [
        ...Npm.definitions({
            bundle,
            readContent,
