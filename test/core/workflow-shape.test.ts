...
test("repository workflow topology admits CI, explicit release paths, and the scoped research probe", () => {
  expect(readdirSync(".github/workflows").filter((name) => name.endsWith(".yml")).sort()).toEqual([
    "ci.yml",
    "pypi-release.yml",
    "refactor-research-probes.yml",
    "release.yml"
  ])
})
...
test("user templates preserve the same handoff, with the environment gate only on publish", () => {
  ...
  for (const value of [automatic, reviewed]) {
    expect(value).toContain("mannyc2/ts-release/apps/ts-release-action@v0.2.2")
