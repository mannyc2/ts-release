...
const sameBasis = (left: DispatchBasis, right: DispatchBasis): boolean => {
  if (left._tag !== right._tag) return false
  if (left._tag === "AcceptedRisk")
    return right._tag === "AcceptedRisk" && left.decisionId === right.decisionId
  if ("dispatchIds" in left)
    return (
      "dispatchIds" in right &&
      left.dispatchIds.length === right.dispatchIds.length &&
      left.dispatchIds.every((id, index) => id === right.dispatchIds[index])
    )
  return true
}
...
type Attempt =
  ...
  | {
      ...
      readonly start: DispatchStarted
...
const entries = (state: OperationState): ReadonlyArray<Attempt> =>
  state.attempts._tag === "NeverStarted" ? [] : state.attempts.entries
...
class TransitionMachine implements Machine {
  ...
  private operation(id: string): OperationState {
    return (
      this.state.operations.get(id) ??
      fail("unknown-operation", "Operation is not registered in this plan")
    )
  }
  ...
  private status(id: string): OperationStatus {
    ...
    if (observed === "Satisfied" || observed === "Conflict" || observed === "Pending")
      return observed
    switch (operation.attempts._tag) {
      ...
      case "Attempted":
        if (
          operation.attempts.entries.some(
            (attempt) => attempt._tag === "Accepted" && attempt.status === "Satisfied",
          )
        )
          return "Satisfied"
        if (operation.attempts.entries.some((attempt) => attempt._tag === "Accepted"))
          return "Pending"
        return operation.attempts.entries.every((attempt) => attempt._tag === "NonCommit")
          ? "Rejected"
          : "Inconclusive"
    }
  }
    ...
    return {
      ...
      operations: this.plan.operations.map((operation) => {
        ...
        return {
          ...
          status: this.status(operation.operationId),
          dispatches: attempts.length,
          receipts: attempts.filter((attempt) => attempt._tag === "Accepted").length,
  ...
  next(operationId: string, candidate: CandidateRequest | null, now: number): Next {
    ...
    switch (state.attempts._tag) {
      ...
      case "Attempted": {
        ...
        if (previous.some((attempt) => attempt._tag === "Accepted"))
          return { _tag: "Finish", status }
        ...
        if (open.length === 0)
          return {
            _tag: "AppendDispatch",
            basis: new NonCommit({
              dispatchIds: previous.map((attempt) => attempt.start.dispatchId).sort(),
            }),
          }
        if (open.every((attempt) => sameProtectedRequest(attempt.start.request, candidate.facts)))
          return {
            _tag: "AppendDispatch",
            basis: new ProtectedReplay({
              dispatchIds: open.map((attempt) => attempt.start.dispatchId).sort(),
            }),
          }
        for (const decision of state.decisions) {
          if (
            !decision.consumed &&
            decision.event.expiresAt >= now &&
            decision.event.fingerprint === candidate.fingerprint &&
            sameStrings(
              decision.event.priorDispatchIds,
              previous.map((attempt) => attempt.start.dispatchId),
            )
          )
            return {
              _tag: "AppendDispatch",
              basis: new AcceptedRisk({ decisionId: decision.event.decisionId }),
            }
        }
  ...
  append(event: JournalEvent): Machine {
    if (event.planId !== this.plan.planId || this.state.eventIds.has(event.eventId))
    ...
    switch (body._tag) {
      ...
      case "ObservationRecorded": {
        ...
        operations.set(body.operationId, {
          ...state,
          observation: body,
          selected: state.selected || body.status === "Satisfied",
          observationCount: state.observationCount + 1,
        })
        break
      }
      case "RiskAccepted": {
        if (this.state.disposition === "Superseded")
        ...
        if (
          !body.principal ||
          previous.length === 0 ||
          previous.some((attempt) => attempt._tag === "Accepted") ||
          !sameStrings(
            body.priorDispatchIds,
            previous.map((attempt) => attempt.start.dispatchId),
          )
        )
          fail("risk-scope", "Risk must bind the complete prior attempt set")
        if (
          [...operations.values()].some((operation) =>
            operation.decisions.some((decision) => decision.event.decisionId === body.decisionId),
          )
        )
          fail("duplicate-risk", "Decision identity is already used")
        operations.set(body.operationId, {
          ...state,
          decisions: [...state.decisions, { event: body, consumed: false }],
        })
        break
      }
      case "DispatchStarted": {
        ...
        if (
          [...operations.values()].some((operation) =>
            entries(operation).some((attempt) => attempt.start.dispatchId === body.dispatchId),
          )
        )
        ...
        if (decision._tag !== "AppendDispatch" || !sameBasis(decision.basis, body.basis))
          fail("illegal-dispatch", "Historical transition lacks a lawful dispatch basis")
        const nextAttempts: readonly [Attempt, ...Attempt[]] = [
          { _tag: "Open", start: body },
          ...entries(state),
        ]
        operations.set(body.operationId, {
          ...state,
          attempts: { _tag: "Attempted", entries: nextAttempts },
          decisions: state.decisions.map((risk) => ({
            ...risk,
            consumed:
              risk.consumed ||
              (body.basis._tag === "AcceptedRisk" &&
                body.basis.decisionId === risk.event.decisionId),
          })),
        })
        break
      }
      ...
      case "DispatchRejectedBeforeCommit": {
        const owner = [...operations.entries()].find(([, state]) =>
          entries(state).some((attempt) => attempt.start.dispatchId === body.dispatchId),
        )
        ...
        const updated = state.attempts.entries.map((attempt): Attempt => {
          ...
          if (attempt._tag !== "Open")
            fail(
              attempt._tag === target ? "duplicate-evidence" : "contradictory-evidence",
              "A terminal attempt cannot transition to another terminal fact",
            )
...
export const transitionMachine: MachineConstructor = (
  plan,
  events,
  scopeKind = "PublicationScope",
) => {
  const initial = new TransitionMachine(
    plan,
    {
      disposition: "Active",
      eventIds: new Set(),
