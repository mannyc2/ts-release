...
export class FixtureObservation extends Schema.Class<FixtureObservation>("FixtureObservation")({
  ...
  if (path)
    appendFileSync(
      ...
      JSON.stringify({
        ...
        encodedBytes: new TextEncoder().encode(canonical(event)).length,
        result,
        backend,
      }) + "\n",
    )
...
export const measureStore = (store: JournalStore, backend: string): JournalStore => ({
  ...
  append: (id, revision, event) =>
    store.append(id, revision, event).pipe(
      Effect.tap((result) => Effect.sync(() => recordEventSize(event, result._tag, backend))),
      Effect.catch((error) => {
        recordEventSize(event, "Rejected", backend)
...
export class MemoryJournal implements JournalStore {
  ...
  append = (planId: string, expected: number, event: JournalEvent) =>
    Effect.try({
      try: (): AppendResult => {
        const events = this.journals.get(planId) ?? []
        const existing = events.findIndex((item) => item.eventId === event.eventId)
        if (existing >= 0) {
          if (canonical(events[existing]) !== canonical(event))
            throw new ReleaseError({
              code: "event-id-conflict",
              message: "Event ID has different bytes",
            })
        ...
        error instanceof ReleaseError
          ? error
          : new ReleaseError({ code: "store", message: String(error) }),
    })
}
...
export const providerFor = (
  ...
  receiptCorresponds: (_operation, request, input) => {
    ...
    return (
      receipt.status === 201 &&
      receipt.endpoint === request.endpoint &&
      receipt.bodyDigest === request.bodyDigest
    )
  },
  prepare: Effect.fn("fixture.prepare")(function* (operation) {
    ...
    return yield* makeRequest({
      ...
      endpoint: `${intent.endpoint}/${intent.coordinate}`,
      method: "PUT",
      headers: [["content-type", "application/octet-stream"]],
      body: new TextEncoder().encode(intent.content),
