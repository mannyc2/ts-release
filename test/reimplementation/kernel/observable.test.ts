  ...
  describe(candidate, () => {
    test("C01: real send follows durable started fact, report is recreated from history", async () => {
      ...
      expect(fixture.sends).toEqual([
        {
          eventCount: 1,
          endpoint: "https://fixture.invalid/package-1",
      ...
      ).toEqual(["DispatchStarted", "ReceiptAccepted"])
    })
    ...
    test("C02b: proven terminal noncommit is linked to dispatch and permits another attempt", async () => {
      const fixture = await makeFixture(
        {
          ...providerFor(),
          rejection: {
            version: "fixture-terminal-rejection/1",
            codec: Schema.Struct({
      ...
      expect(
        (await runWithHost(rejected, runRelease({ plan: fixture.plan, authorize: true })))
          .operations[0]?.status,
      ).toBe("Rejected")
      const result = await runWithHost(
        fixture.host,
      ...
      const host: HostShape = {
        ...
        transport: {
          send: () =>
            Effect.sync(() => {
              sends++
              return { _tag: "Unknown", reason: "connection lost" } as const
            }),
        },
      }
    ...
    test("C05: only identical structural Git CAS can replay a possibly committed attempt", async () => {
      ...
      const git = {
        ...provider,
        receiptVersion: "git/1",
        receiptCodec: GitReceipt,
        receiptCorresponds: (
          _op: Operation,
          request: import("./kernel.js").RequestFacts,
          receipt: unknown,
        ) =>
        ...
        prepare: () =>
          makeRequest({
            ...
            endpoint: "file:///fixture/remote.git",
            method: "update-ref",
            headers: [],
            body: new Uint8Array(),
            principal: "git-user",
          ...
          execute: (args) =>
            Effect.sync(() => {
              ...
              expect(args).toEqual([
                "push",
                "--porcelain",
                `--force-with-lease=refs/heads/release:${"0".repeat(40)}`,
                "--",
                "file:///fixture/remote.git",
                `${"1".repeat(40)}:refs/heads/release`,
              ])
              return calls === 1
                ? { exitCode: 1, stdout: "" }
                : { exitCode: 0, stdout: `=\t${"1".repeat(40)}:refs/heads/release\t[up to date]\n` }
            }),
        }),
      }
      await runWithHost(host, runRelease({ plan: fixture.plan, authorize: true }))
    ...
    test("C06: authenticated risk binds request, all prior attempts, expiry and one additional attempt", async () => {
      ...
      expect(sends).toBe(2)
      expect((await startEvents(fixture.store, fixture.plan))[1]?.body).toMatchObject({
        basis: { _tag: "AcceptedRisk", decisionId: "approval-1" },
      })
    })
    ...
    test("C08: request endpoint, bytes, headers, principal and scope drift block protected replay", async () => {
      ...
      const host: HostShape = {
        ...
        transport: makeCoreGitTransport({
          principal: "a",
          scope: "ref-a",
          execute: () => Effect.succeed({ exitCode: 1, stdout: "" }),
        }),
      }
      await runWithHost(host, runRelease({ plan: fixture.plan, authorize: true }))
      for (const changed of [
        { endpoint: "file:///other.git" },
        { principal: "b" },
        { scope: "ref-b" },
        { headers: [["x-law", "changed"]] as [string, string][] },
      ]) {
        const altered = await Effect.runPromise(
          makeRequest({ ...original.facts, ...changed, body: original.body }),
        )
        const changedRun = runWithHost(
          { ...host, providers: [{ ...provider, prepare: () => Effect.succeed(altered) }] },
          runRelease({ plan: fixture.plan, authorize: true }),
        )
    ...
    test("C09: supersession accepts late factual receipt while forbidding new dispatch", async () => {
      ...
      expect(events.map((event) => event.body._tag)).toEqual([
        "DispatchStarted",
        "PlanSuperseded",
        "ReceiptAccepted",
      ])
              ...
              .pipe(
                Effect.map((result) =>
                  event.body._tag === "DispatchStarted"
                    ? resultTag === "AlreadyRecorded"
                      ? { _tag: resultTag, revision: 1 }
                      : { _tag: resultTag }
                    : result,
                ),
              ),
    ...
    test("C12: one definition serves two distinct instance intents without ID collision", async () => {
      ...
      expect(fixture.sends.map((send) => send.endpoint).sort()).toEqual([
        "https://fixture.invalid/package-1",
        "https://other.invalid/package-1",
      ])
    })
      ...
      const host: HostShape = {
        ...
        transport: {
          send: () =>
            Effect.sync(() => {
              calls++
              return { _tag: "Unknown", reason: "accepted before identifier recorded" } as const
            }),
        },
      }
      await runWithHost(host, runRelease({ plan: fixture.plan, authorize: true }))
...
test("M1/M2 reject impossible histories, preserve late evidence, and expose equal reports", async () => {
  ...
  const envelope = (eventId: string, body: JournalEvent["body"]) =>
    new JournalEvent({
  ...
  for (const constructor of Object.values(evaluators))
    expect(constructor(fixture.plan, history).report()).toEqual(
      historyMachine(fixture.plan, history).report(),
    )
})
