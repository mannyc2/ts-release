      ...
      test(`${candidate}${cache ? "+cache" : ""}: fresh OS process resumes ${fault} from SQLite after deleting original workspace`, async () => {
        ...
        try {
          const operation = await Effect.runPromise(
            createOperation(providerFor(), {
              coordinate: "package-1",
              endpoint: `http://127.0.0.1:${server.port}`,
              content: "owned exact artifact bytes",
            }),
          )
          const plan = await Effect.runPromise(createPlan("process-owned-bundle", [operation]))
          writeFileSync(planPath, canonical(plan))
          const launch = (cwd: string, stage: string, observe: string) =>
            Bun.spawn(
              [process.execPath, worker, planPath, databasePath, candidate, stage, observe],
              {
                cwd,
                stdout: "pipe",
                stderr: "pipe",
                env: { ...process.env, LAB_CACHE: cache ? "1" : "" },
              },
            )
