...
for (const candidate of evaluatorNames) for (const competitor of [false, true]) {
  test(`${candidate}: real native conditional Git replay ${competitor ? "rejects a competing update" : "confirms an already committed update"}`, async () => {
    ...
    try {
      ...
      const provider = {
        ...providerFor(), receiptVersion: "git-push/1", receiptCodec: GitReceipt,
        receiptCorresponds: (_operation: Operation, request: RequestFacts, native: unknown) => {
          const receipt = native as GitReceipt
          return request.replay._tag === "GitCas" && receipt.ref === request.replay.ref && receipt.desiredNew === request.replay.desiredNew
        },
      ...
      const host: HostShape = {
        ...fixture.host,
        transport: makeCoreGitTransport({ principal: "local-fixture", scope: ref, execute: (args) => Effect.sync(() => {
          commands.push(args)
          const result = Bun.spawnSync(["git", "-C", source, ...args], { stdout: "pipe", stderr: "pipe" })
      ...
      expect(readFileSync(join(remote, "logs", ref), "utf8").trim().split("\n")).toHaveLength(competitor ? 3 : 2)
