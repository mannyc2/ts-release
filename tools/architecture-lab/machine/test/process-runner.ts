...
const provider = {
  ...providerFor(), observationVersion: "http-bytes/1",
  ...
  classifyObservation: (_operation: unknown, evidence: unknown) => {
    ...
    return native.status === 404 ? "Absent" as const : native.status === 200 && native.bodyDigest === expectedDigest ? "Satisfied" as const : "Conflict" as const
  },
  observe: () => Effect.gen(function*() {
    const response = yield* Effect.tryPromise({ try: () => fetch(`${intent.endpoint}/${intent.coordinate}`), catch: () => new LabError({ code: "fixture-http", message: "Observation request failed" }) })
  ...
  transport: {
    send: (request) => Effect.gen(function*() {
      const response = yield* Effect.tryPromise({
        try: () => fetch(request.facts.endpoint, { method: request.facts.method, body: new Uint8Array(request.body) }),
        catch: () => new LabError({ code: "fixture-http", message: "Mutation request failed" })
      })
      return { _tag: "Accepted" as const, receipt: { status: response.status, endpoint: request.facts.endpoint, bodyDigest: request.facts.bodyDigest } }
    })
  }
}
try {
