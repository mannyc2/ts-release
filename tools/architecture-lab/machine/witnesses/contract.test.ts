...
import { PROVIDER_CONTRACT, runRelease, reportRelease, type ProviderDefinition } from "../src/index.js"
import { makeFixture, providerFor, runWithHost } from "../test/fixtures.js"
...
test("two physical kernel copies of the same contract interoperate at the data level (the hazard is version skew, not duplication itself)", async () => {
  ...
  try {
    cpSync(join(import.meta.dir, "../src"), join(directory, "src"), { recursive: true })
    const copy = await import(join(directory, "src/index.ts")) as typeof import("../src/index.js")
    expect(copy.PROVIDER_CONTRACT).toBe(PROVIDER_CONTRACT)
    expect(copy.NoReplay).not.toBe((await import("../src/index.js")).NoReplay) // distinct module instances
    const fixture = await makeFixture()
