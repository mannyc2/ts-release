...
const implementedAuthorities = ["kernel", "application", "npm", "pypi", "github", "homebrew", "scoop", "artifactReader", "http", "git", "nodeHost", "bunHost"];
...
const surfaces: Surface[] = [
    ...
    { id: "core.git", entry: "kernel.Git", symbols: picks("git", named("git"), "kernel.Git") },
    { id: "core.node", entry: "host.Node", symbols: picks("nodeHost", named("nodeHost"), "host.Node") },
    { id: "core.bun", entry: "host.Bun", symbols: picks("bunHost", named("bunHost"), "host.Bun") }
];
...
for (const alternative of design.alternatives) {
    ...
    for (const module of modules) {
        ...
        if (module.zone === "provider")
            assert(reachable.every((item) => item.zone === "neutral" || item.zone === "provider" && item.owner === module.owner), `Sibling/host leak ${module.id}`);
    ...
    const coreManifest: Manifest = { ...commonManifest, name: design.rootPackage, bin: { "ts-release": "./dist/bin/ts-release.js" }, exports: {}, dependencies: { "effect-build": design.pins["effect-build"]!, "undici": design.pins["undici"]! }, peerDependencies: Object.fromEntries(["effect", "@effect/platform-node", "@effect/platform-bun", "effect-build-apple"].map((name) => [name, design.peerPolicy[name]])), peerDependenciesMeta: { "@effect/platform-node": { optional: true }, "@effect/platform-bun": { optional: true }, "effect-build-apple": { optional: true } } };
