...
const emissionPath = `${handoff}/production-api/emission.json`;
const emissionText = await read(emissionPath);
const productionEmission = JSON.parse(emissionText) as { records: { declaration: string; declarationSha256: string; source: string; sourceSha256: string }[] };
for (const record of productionEmission.records) {
    assert(hash(await read(`${handoff}/production-api/${record.declaration}`)) === record.declarationSha256, `Stale production declaration binding: ${record.declaration}`);
    assert(hash(await read(record.source)) === record.sourceSha256, `Stale production source binding: ${record.source}`);
}
const productionDeclarations = { path: emissionPath, sha256: hash(emissionText), files: productionEmission.records.length, meaning: "Complete actual declaration-owner files, including private Schema bases, and exact source hashes; regenerated by scripts/project-production-api.ts." };
...
const surfaces: Surface[] = [
    ...
    { id: "core.bundle", entry: "kernel.Bundle", symbols: named("bundle").map(name => {
            const owner = Object.entries(bundleOwners).find(([, names]) => names.includes(name))?.[0];
            assert(owner, `Missing actual Bundle export owner: ${name}`);
            return pick("bundle", name, owner);
        }) },
    { id: "core.effect-build", entry: "kernel.EffectBuild", symbols: picks("effectBuild", named("effectBuild"), "kernel.EffectBuild") },
    { id: "core.apple", entry: "kernel.Apple", symbols: named("apple").map((name) => pick("apple", name, name === "preparationProvider" ? "kernel.AppleProvider" : appleNative.includes(name) ? "kernel.AppleNative" : applePreparation.includes(name) ? "kernel.ApplePreparation" : "kernel.AppleModel")) },
...
for (const alternative of design.alternatives) {
    ...
    const closure = (id: string, active: string[] = []): Set<string> => {
        ...
        return new Set([id, ...[...module.dependencies.filter(child => !module.typeOnlyDependencies?.includes(child)), ...(module.dynamicDependencies ?? [])].flatMap((child) => [...closure(child, [...active, id])])]);
    };
    ...
    const sourceEdges = modules.flatMap((module) => [...module.dependencies.map((target) => ({ target, loading: module.typeOnlyDependencies?.includes(target) ? "type-only" : "static" })), ...(module.dynamicDependencies ?? []).map((target) => ({ target, loading: "dynamic-runtime-choice" }))].map(({ target, loading }) => {
    ...
    const importPolicy = { edgeMeaning: "Direct module dependencies distinguish whole-declaration erased type imports from runtime imports for implemented core owners. Provider and remaining proposal edges retain their declared classification; native installed closure remains separately measured.", sourceEdgesSha256: hash(json(sourceEdges)), packageEdgesSha256: hash(json(packageEdges)), sourceEdges, packageEdges, externalImports: modules.flatMap((module) => (module.externalImports ?? []).map((specifier) => ({ from: module.id, specifier }))), universalDependency: { specifier: "effect", version: design.pins.effect, ownership: "Required peer available to every module; each implementation's exact import list is qualified after emission." }, dynamicApplications: modules.filter((module) => module.dynamicBoundary).map((module) => ({ from: module.id, rule: module.dynamicBoundary })) };
